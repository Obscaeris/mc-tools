...
function NavItem({ title, href, icon, active, target }) {
    ...
    return (React.createElement("div", { className: classNames("flex text-center items-center content-center", "w-full p-1 m-1 my-1 md:mx-2 rounded select-none cursor-pointer", "hover:bg-primary hover:text-background", itemStyle), onClick: () => target === "_self" ? nav(href) : open(href, target) },
